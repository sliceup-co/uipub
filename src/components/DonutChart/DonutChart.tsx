...
const DonutChart = (props: IDonutChartProps) => {
  ...
  const memoizedDrawCallback = useCallback(() => {
    const pieGroup = d3.selectAll('#donut-chart-group')
    pieGroup.style('opacity', '0')
  ...
  const memoizedUpdateCallback = useCallback(() => {
    ...
    const bounds = d3.select('#donut-bounds')

    const margins = props.dimensions.margin.left + props.dimensions.margin.right
    const radius = Math.min(props.dimensions.width, props.dimensions.height) / 2 - margins

    const pieSvg = bounds
      .select('#donut-chart-group')
      .append('svg')
      .attr('width', props.dimensions.width)
      .attr('height', props.dimensions.height)
      .append('g')
      .attr('transform', `translate(${props.dimensions.width / 2 - margins},${props.dimensions.height / 2})`)
    ...
    d3.selectAll('#donut-chart-group')
      .transition()
      .delay(200)
      .style('opacity', '1')
  ...
  useEffect(() => {
    memoizedDrawCallback()
    memoizedUpdateCallback()
  }, [memoizedDrawCallback, memoizedUpdateCallback, props.data])
  ...
  return (
    <div id="donut-div">
      <svg id="wrapper" width={props.dimensions.width} height={props.dimensions.height}>
        <g id="donut-bounds" style={{ transform: `translate(${props.dimensions.margin.left}px, ${props.dimensions.margin.top}px)` }}>
          <g id="donut-chart-group" />
        </g>
      </svg>
    </div>
  )
